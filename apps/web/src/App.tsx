...
export function App() {
  ...
  if (workspace === "knowledge") return <KnowledgeChangeRequests client={client} onBack={() => setWorkspace("architecture")} />;
  ...
  return <>{error && <main><p role="alert">{error}</p></main>}<button onClick={() => setWorkspace("knowledge")}>Knowledge Authoring</button><AnalysisHistory analyses={analyses} onSelect={(id) => { void load(id); }} onNewAnalysis={() => { setError(undefined); setScreen("new"); }} /></>;
}
