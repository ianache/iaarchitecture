...
export function buildApp(dependencies: ApiDependencies): FastifyInstance {
  ...
  app.post<{ Params: { id: string } }>("/analyses/:id/regenerate", async (request, reply) => { try { const regenerated = await analysisService.regenerate(request.params.id); return reply.code(201).send({ id: regenerated.id, status: regenerated.result?.packageStatus.value ?? regenerated.status, generation: regenerated.result?.generation }); } catch (error) { const response = errorResponse(error); return reply.code(response.status).send(response.body); } });
