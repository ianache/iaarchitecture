...
export class RetrievalService implements EvidenceRetriever {
  ...
  async retrieve(input: RetrieveInput): Promise<RetrievedEvidence[]> {
    if (input.revision !== this.store.getRevision()) throw new Error(`Projection revision ${this.store.getRevision()} does not match requested ${input.revision}`);
    ...
    const results = await Promise.all(selected.map(async (item) => {
      ...
      return { id: `EVIDENCE-${item.id}`, knowledgeId: item.id, sourcePath: item.sourcePath, revision: item.revision, excerpt: item.content ?? item.summary, classification: item.type, confidence: Math.min(1, score), method: keyword ? "full-text+vector" : "vector", score } satisfies RetrievedEvidence;
    }));
