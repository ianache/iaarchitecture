...
export interface ApiDependencies { orchestrator: ArchitectureOrchestrator; knowledgeRevision?: string; analysisRepository?: AnalysisRepository; reviewRepository?: ReviewRepository; analysisService?: AnalysisService; packageService?: PackageService & PackageReadService; governanceService?: GovernanceService; }

function errorResponse(error: unknown): { status: number; body: { code: string; message: string } } {
  if (error instanceof ApplicationError) { const code = error.code as string; return { status: code === "NOT_FOUND" ? 404 : code === "INVALID_REVIEW_TRANSITION" || code === "PACKAGE_NOT_READY" ? 409 : 500, body: { code: error.code, message: error.message } }; }
